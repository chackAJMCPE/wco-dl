...
class Library:
    """
    Tracks downloaded episodes and manages a Jellyfin-compatible folder structure.
        ...
        <download_folder>/
            ...
            <Show Name>/
                ...
                Season 00/
                    ...
                    Show Name - S00E01 - Movie 01 [English].mp4

    Tracking:
        <download_folder>/downloaded.json

        {
            "https://wco.tv/episode-url":
                "Show Name/Season 01/Show Name - S01E01 - Episode 1 [Japanese].mp4"
        }
    """
    ...
    def __init__(self, config: Config):
        ...
        # Keep the tracker with the media library.
        ...
        if self._path.exists():
            try:
                self._data: dict[str, str] = json.loads(
    ...
    def _fmt_ep(ep_no: str) -> str:
        """
        Format episode number for Jellyfin:
    ...
    def build_path(
        ...
        """Return the full destination path for an episode file."""
        show_s = self._safe(show)
        show_dir = self.root / show_s

        # Extract the episode number from:
        #   Episode 1
        #   Episode 1A
        #   Episode 24.5
        #   OVA 2
        ...
        ep_fmt = self._fmt_ep(ep_raw)

        if season == "OVA":
            season_dir = show_dir / "Season 00"

            filename = (
                f"{show_s} - S00E{ep_fmt} - "
                f"OVA {ep_fmt} [{language}].mp4"
            )
            ...
            season_dir = (
                show_dir / f"Season {snum}"
            )

            ep_title = self._safe(episode)

            filename = (
                f"{show_s} - S{snum}E{ep_fmt} - "
                f"{ep_title} [{language}].mp4"
            )
    ...
    def is_downloaded(
        ...
        """
        Return the relative filepath if the episode is still present.

        A stale downloaded.json entry whose media file was deleted is treated
        as not downloaded, allowing the episode to be downloaded again.
        """
        with self._lock:
            relative = self._data.get(url)
    ...
    def mark_downloaded(
        ...
        try:
            ...
            raise RuntimeError(
                f"Downloaded file is outside library root: {dest}"
            ) from e
    ...
    def cleanup_parts(self):
        for f in self.root.rglob("*.part"):
            try:
                f.unlink()
                logger.debug(
                    f"  Cleaned up orphaned file: {f.name}"
                )
            except OSError as e:
                logger.debug(
                    f"  Could not remove orphaned file "
                    f"{f.name}: {e}"
                )
...
class Network:
    ...
    def raw_get(
        ...
        raise requests.HTTPError(
            f"HTTP {r.status_code} for {url[:100]} "
            f"(final URL: {r.url[:120]})"
        )
    ...
    def prime_server(
        ...
        try:
            ...
            self._primed_servers.add(
                server_url
            )

            logger.debug(
                f"  CDN primed: {server_url}"
            )

        except Exception as e:
            logger.debug(
                f"  CDN priming failed "
                f"(non-fatal): {e}"
            )
    ...
    def download_file(
        ...
        # A ranged request returning 200 means the server ignored Range.
        # Do not append a complete file to a partial file.
        if (
            resume_from
        ...
        if r.status_code not in (
            ...
            raise RuntimeError(
                f"Unexpected media HTTP status "
                f"{r.status_code}\n"
                f"  Requested URL: {url}\n"
                f"  Final URL: {r.url}"
            )
    ...
    def download_hls(
        ...
        if not shutil.which("ffmpeg"):
            raise RuntimeError(
                "ffmpeg is required for HLS streams "
        ...
        referer = (
            ...
            if (
                "watchanimesub" in url
                or "cizgifilmlerizle" in url
            )
...
def parse_series_name(
    ...
    if "anime" in parts:
        ...
        if idx + 1 >= len(parts):
            raise ValueError(
                f"Could not determine series name "
                f"from URL: {url}"
            )
...
def parse_episode_meta(
    ...
    show_raw = (
        text[:name_end.start()].strip()
        if name_end
        else text.strip()
    )

    show = title_from_slug(
        show_raw
    )
...
class Scraper:
    ...
    def get_episodes(
        ...
        html = self.net.get_rendered_page(
            url
        )
    ...
    def get_embed_url(
        ...
        html = self._fetch_episode_page(
            episode_url
        )

        src = self._extract_iframe_src(
            html,
            episode_url,
        )
    ...
    def _fetch_episode_page(
        ...
        try:
            ...
            logger.debug(
                f"  HTTP episode fetch failed: {e}"
            )

            return ""

    def _extract_iframe_src(
        self,
        html: str,
        episode_url: str,
    ) -> str | None:
    ...
    def _is_delivery_node_url(
        ...
        return Network._is_delivery_node(
            host
        )
    ...
    def _extract_legacy(
        ...
        file_param = query.get(
            "file",
            [""],
        )[0]

        embed_param = query.get(
            "embed",
            [""],
        )[0]
        ...
        if pid_param:
            params.append(
                f"pid={quote(pid_param, safe='')}"
            )

        if h_param:
            params.append(
                f"h={quote(h_param, safe='')}"
            )

        if t_param:
            params.append(
                f"t={quote(t_param, safe='')}"
            )
        ...
        try:
            ...
            logger.debug(
                f"  index.php loaded — "
                f"{len(dict(self.net.session.cookies))} "
                f"cookie(s) set"
            )

        except Exception as e:
            logger.debug(
                f"  index.php preload failed "
                f"(non-fatal): {e}"
            )
        ...
        server = self._normalise_origin(
            data.get("server")
        )

        cdn = self._normalise_origin(
            data.get("cdn")
        )
        ...
        logger.debug(
            ...
            f"enc={enc[:10]}... "
            f"hd={bool(hd_token)} "
            f"fhd={bool(fhd_token)} "
            f"sub={bool(sub_token)}"
        )
    ...
    def _resolve_json_token(
        ...
        for base in candidates:
            ...
            try:
                ...
                if self._is_wco_capacity_host(
                    host
                ):
                    logger.debug(
                        "  SD resolver returned "
                        f"capacity host: {host}"
                    )
    ...
    def _build_token_url(
        ...
        return (
            f"{server.rstrip('/')}/getvid?"
            f"evid={quote(token, safe='')}"
        )
    ...
    def _extract_saturn(
        ...
        return [
            ...
            },
            {
                "label": "720p",
                "url": (
                    f"{prefix}1280{suffix}"
                ),
            },
            {
                "label": "1080p",
                "url": (
                    f"{prefix}1920{suffix}"
                ),
            },
        ]
...
class ErrorLog:
    ...
    def print_summary(self):
        if self._entries:
            print(
                f"\n{C.RED}"
                f"Errors occurred during download "
                f"of some episodes. "
                f"Check the specifics in errors.txt"
                f"{C.RESET}"
            )
...
def download_episode(
    ...
    try:
        embed_url = scraper.get_embed_url(
            url
        )
        ...
        if "Premium episode" in str(e):
            return (
                False,
                "⏭  Skipped  — premium only",
                "",
            )

        detail = (
            f"Could not load page: {e}\n"
            f"{traceback.format_exc()}"
        )

        return (
            False,
            "✗  Failed   — could not load page",
            detail
            + (
                f"\n\nDebug log:\n{debug_log}"
                if debug_log
    ...
    existing = library.is_downloaded(
        url
    )
    ...
    source = scraper.select_resolution(
        sources,
...
def main(
    ...
    episode: bool = typer.Option(
        ...
        "--episode",
        "-de",
        help="Download a single episode URL",
    ),
    series: bool = typer.Option(
        False,
        "--series",
        "-ds",
        help="Download a full series URL",
    ),
    all_series: bool = typer.Option(
        False,
        "--all",
        "-da",
        help="Download all series from list file",
    ),
    ...
    workers: int = typer.Option(
        ...
        "-w",
        help=(
            "Number of parallel episode downloads "
            "(default: 6)"
        ),
    ),
):
        ...
        if search:
            ...
            if not results:
                ...
                for path in results:
                    ...
                    print(
                        f"  https://www.wco.tv{path}\n"
                    )

        elif episode:
            if not target:
                typer.echo(
                    "--episode requires a URL"
                )
                raise typer.Exit(1)
            ...
            print(
                f"Found {len(urls)} series in list.\n"
            )
